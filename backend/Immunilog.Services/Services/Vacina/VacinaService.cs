...
public interface IVacinaService
{
    ...
    Task<List<Immunilog.Domain.Entities.Vacina>> GetVacinaByIdadePessoa(Guid id, string param);
...
public class VacinaService : IVacinaService
{
    ...
    public async Task<List<Immunilog.Domain.Entities.Vacina>> GetVacinaByIdadePessoa(Guid id, string param)
    {
        var listVacina = await vacinaRepository.GetVacinaByIdadePessoa(id, param);
        return listVacina;
    }
